...
/**
 * Spring Data JPA repository for {@code shipment_requests}. Default queries respect the
 * {@code @SQLRestriction("deleted_at IS NULL")} filter on {@link ShipmentRequest}; native methods
 * below bypass it for admin / purge paths.
 */
public interface ShipmentRequestRepository
        extends JpaRepository<ShipmentRequest, UUID>, JpaSpecificationExecutor<ShipmentRequest> {
    ...
    Page<ShipmentRequest> findByUserIdAndStatus(
            String userId, ShipmentStatus status, Pageable pageable);
    ...
    @Query(
            value =
                    "SELECT id FROM shipment_requests WHERE deleted_at IS NOT NULL AND deleted_at < :cutoff",
            nativeQuery = true)
