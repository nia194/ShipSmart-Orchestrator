...
/**
 * Honors inbound X-Request-Id and W3C traceparent; mints them if absent. Runs before JwtAuthFilter
 * so requestId is in MDC for every log line. Outbound RestTemplate/WebClient interceptors forward
 * the same headers so Python / MCP see the same correlation IDs (logs grep cleanly across
 * services).
 */
@Component
