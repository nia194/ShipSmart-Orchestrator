...
/**
 * Loads environment variables from .env file at application startup. This component must be
 * initialized before DataSource configuration.
 */
@Component
public class EnvLoader {
    ...
    public void loadEnv() {
        try {
            ...
            log.info(
                    "Environment variables loaded from .env file (overriding inherited shell env)");
        } catch (DotenvException e) {
            log.warn(
                    "No .env file found or error reading it (OK for production): {}",
                    e.getMessage());
        }
    }
}
