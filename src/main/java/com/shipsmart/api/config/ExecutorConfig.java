...
/**
 * Bounded, observable executors for the two real multi-threaded paths: - Quote-provider fanout
 * (outbound HTTP; short-lived; timeout-protected). - Audit writes (fire-and-forget; never drop
 * under backpressure).
 *
 * <p>Both wrap {@link MdcAwareExecutor} so logs keep the right requestId/traceId.
 */
@Configuration
@EnableAsync
@EnableScheduling
public class ExecutorConfig {
    ...
    static final class MdcAwareExecutor extends AbstractExecutorService {
        ...
        MdcAwareExecutor(ExecutorService delegate) {
