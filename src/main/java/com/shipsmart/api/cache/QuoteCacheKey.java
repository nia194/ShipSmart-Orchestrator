 ...
 *
 * <ul>
 *   <li>{@code ProviderQuoteRequest} carries a {@code double totalWeight}. Using a double directly
 *       in equals/hashCode is unsafe (NaN, ±0). We bucket weight to the nearest kg here — quotes
 *       for 10.01 kg and 10.03 kg should hit the same cache entry.
 *   <li>The key implements {@link Comparable} so it can be indexed in a {@code TreeSet}/{@code
 *       TreeMap} (used by the {@link QuoteCache#keysSortedByRoute()} helper).
 * </ul>
 *
 * <p>Style: this is a classic Value Object — final class, final fields, explicit
 * equals/hashCode/toString, no setters, defensive normalization in the constructor. Swapping to a
 * record would work too, but keeping this as a final class lets us document the invariants inline.
 */
public final class QuoteCacheKey implements Comparable<QuoteCacheKey> {
    ...
    public QuoteCacheKey(
        ...
        this.expectedDeliveryDate =
                Objects.requireNonNull(expectedDeliveryDate, "expectedDeliveryDate");
    ...
    public int weightBucketKg() {
        return weightBucketKg;
    }
    ...
    public int hashCode() {
        return Objects.hash(
                origin, destination, dropOffDate, expectedDeliveryDate, weightBucketKg, totalItems);
    }
    ...
    /**
     * Natural ordering: origin, then destination, then drop-off date. Lets a {@link
     * java.util.TreeMap} view keys in a stable, readable order — handy for the cache dump endpoint
     * (future work).
     */
    @Override
