 ...
 *
 * <p>Extracts the user ID (sub claim) from the JWT in the Authorization header and stores it in the
 * Spring SecurityContext. Runs on every request but does NOT enforce auth — that is handled by the
 * SecurityFilterChain rules.
 *
 * <p>Supports multiple verification modes: 1. JWKS (public key) — for Supabase ES256 tokens
 * (production) 2. HMAC (symmetric secret) — fallback for simple secrets 3. Unsafe extraction — for
