 ...
 *
 * <p>Supports two verification modes: 1. JWKS (public key) — for Supabase ES256 tokens, fetched
 * from issuer's JWKS endpoint 2. HMAC (symmetric secret) — only for HS256-signed tokens
 * (legacy/test mode)
 *
 * <p>JWKS keys are cached in memory and refreshed if a key ID is not found. ES256 tokens are
 * verified exclusively with public keys; HMAC fallback only applies to HS256.
 */
@Component
public class SupabaseJwtVerifier {
    ...
    /**
     * Verify a JWT token and extract the subject (user ID). Returns null if verification fails.
     *
     * <p>Strategy: 1. Inspect token algorithm from header 2. If ES256 (ECDSA): require JWKS
     * verification; do NOT fall back to HMAC 3. If HS256 (HMAC): verify with shared secret if
     * configured 4. Otherwise: reject (return null) — never trust an unverified token
     */
    ...
    /**
     * Verify token using JWKS (ES256). Returns subject if valid, null otherwise.
     *
     * <p>Derives JWKS URL from JWT payload's 'iss' claim (issuer), e.g.,
     * iss="https://project.supabase.co/auth/v1" → JWKS at /.well-known/jwks.json
     */
    private String verifyWithJwks(String token) throws Exception {
        ...
        if (publicKey == null) {
            log.warn(
                    "JWKS verification failed: could not find public key for kid={} from issuer={}",
                    kid,
                    issuer);
            return null;
        }
        ...
        log.debug(
                "Found matching public key (kid={}, type={}); verifying ES256 signature",
                kid,
        ...
        try {
            ...
            log.info(
                    "ES256 token signature verified successfully: kid={}, user={}, issuer={}",
                    kid,
                    claims.getSubject(),
                    issuer);
            ...
            log.warn(
                    "ES256 signature verification failed for token from issuer={}, kid={}: {}",
                    issuer,
                    kid,
                    e.getMessage());
            return null;
    ...
    /**
     * Construct JWKS URL from issuer URL. Supabase issuer format:
     * https://project.supabase.co/auth/v1 JWKS URL:
    ...
    /** Verify token using HMAC (HS256). Returns subject if valid, null otherwise. */
    ...
    /**
     * Fetch JWKS from the specified URL and cache all public keys. Supabase Auth JWKS endpoint:
     * https://project.supabase.co/auth/v1/.well-known/jwks.json
     */
    private boolean fetchAndCacheJwks(String jwksUrl) {
        try {
            ...
            java.util.List<Map<String, Object>> keys =
                    (java.util.List<Map<String, Object>>) keysObj;
            ...
            for (Map<String, Object> keyData : keys) {
                ...
                try {
                    ...
                    if ("RSA".equals(keyType)) {
                        ...
                        ecKeyCount++;
                        log.debug(
                                "Cached EC-{} public key kid={} curve={}",
                                keyData.get("alg"),
                                keyId,
                                keyData.get("crv"));
                    }
                    ...
                    log.warn(
                            "Failed to build public key for kid={} (kty={}): {}",
                            keyId,
                            keyType,
                            e.getMessage());
                }
            ...
            log.info(
                    "Successfully fetched and cached {} JWKS keys ({} RSA, {} EC) from {}",
                    keyCount,
                    rsaKeyCount,
                    ecKeyCount,
                    jwksUrl);
    ...
    /** Build PublicKey from JWK (RSA or EC). Supabase typically uses EC keys for ES256 tokens. */
    ...
    /**
     * Build EC PublicKey from JWK components (x, y, crv). Supabase ES256 tokens use P-256 curve
     * (crv="P-256").
     *
     * <p>Uses JJWT's built-in EC key construction which handles curve mapping automatically.
     */
    private PublicKey buildEcPublicKey(Map<String, Object> keyData) throws Exception {
        ...
        java.security.spec.ECPublicKeySpec ecPublicKeySpec =
                new java.security.spec.ECPublicKeySpec(ecPoint, ecParams);
    ...
    /**
     * Get EC parameter spec for standard NIST curves. Uses KeyPairGenerator to initialize and
     * extract the parameter spec.
     */
