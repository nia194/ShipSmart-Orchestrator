 ...
 *
 * <p>Study note — <b>enum with per-constant behavior</b>: each enum constant overrides {@link
 * #comparator()} with a different strategy. This is the "enum as mini state machine / strategy
 * dispatch" pattern. The API is type-safe (no stringly-typed sort modes) and adding a new option
 * becomes impossible to forget about — the enum won't compile without an implementation.
 */
public enum QuoteSortOption {
    CHEAPEST {
        @Override
        public Comparator<ProviderQuote> comparator() {
