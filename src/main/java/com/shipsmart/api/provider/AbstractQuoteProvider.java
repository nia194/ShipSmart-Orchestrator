 ...
 *
 * <ul>
 *   <li><b>Abstract class</b> (vs interface-only) — lets us hoist cross-cutting behavior (logging,
 *       metrics, disabled short-circuit) out of each subclass. Interfaces can't hold mutable shared
 *       state or final method templates with fallback behavior in the same way.
 *   <li><b>{@code final} template method</b> — {@link #quote} is marked {@code final} so subclasses
 *       can't accidentally bypass the metrics/try-catch envelope when overriding.
 *   <li><b>Protected hook</b> — {@link #callCarrier(ProviderQuoteRequest)} is the single extension
 *       point subclasses must implement.
 *   <li><b>{@code @Autowired} setter</b> — optional dependency; lets metrics be null in narrow unit
 *       tests that instantiate a subclass directly without Spring. Constructor injection would
 *       force every subclass to propagate the dependency.
 * </ul>
 */
public abstract class AbstractQuoteProvider implements QuoteProvider {
    ...
    public final List<ProviderQuote> quote(ProviderQuoteRequest request) {
        if (!isEnabled()) {
            log.debug(
                    "Provider {} disabled (missing credentials); returning empty quotes",
                    carrierCode());
    ...
    private void recordEvent(ProviderCallOutcome outcome, long latencyMs, int count) {
        ...
        metrics.record(
                new ProviderCallEvent(carrierCode(), outcome, latencyMs, count, Instant.now()));
    }
