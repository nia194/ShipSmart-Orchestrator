...
/**
 * Bean-lifecycle demo: on {@link InitializingBean#afterPropertiesSet()}, enumerates every {@link
 * QuoteProvider} bean and logs enabled / disabled status. Fails fast only if ZERO providers are
 * enabled — a misconfigured Java plane should not serve (silently-empty) quote lists in production.
 *
 * <p>{@link #enabled()} returns providers sorted by {@link QuoteProvider#priority()} (lower first)
 * so the fanout service consults fast/preferred carriers before slower ones. A stable secondary
 * sort on carrier code keeps the order deterministic for equal-priority providers — essential for
 * log diffing.
 */
@Component
public class QuoteProviderRegistry implements InitializingBean {
    ...
    public void afterPropertiesSet() {
        ...
        log.info(
                "QuoteProviderRegistry: {} of {} providers enabled [{}]",
                enabled.size(),
                providers.size(),
                summary);
