 ...
 *
 * <ul>
 *   <li>Consults the {@link QuoteCache} (LRU, TTL-bounded) before dispatching carrier calls. Cache
 *       hits skip the executor entirely.
 *   <li>Uses the dedicated {@code quoteProviderExecutor} (bounded pool, observable metrics).
 *   <li>Per-call {@code orTimeout} prevents one slow carrier from stalling the request.
 *   <li>Failures downgrade to an empty list — never fail the whole request for one provider.
 *   <li>Exposes {@link #fanoutSorted} so callers can request a canonical ordering (cheapest /
 *       fastest / recommended).
 * </ul>
 */
@Service
public class QuoteFanoutService {
    ...
    public List<ProviderQuote> fanout(ProviderQuoteRequest request) {
        ...
        List<CompletableFuture<List<ProviderQuote>>> futures =
                providers.stream()
                        .map(
                                p ->
                                        CompletableFuture.supplyAsync(
                                                        () -> p.quote(request),
                                                        quoteProviderExecutor)
                                                .orTimeout(3, TimeUnit.SECONDS)
                                                .exceptionally(
                                                        ex -> {
                                                            log.warn(
                                                                    "Provider {} timed out or failed: {}",
                                                                    p.carrierCode(),
                                                                    ex.getMessage());
                                                            return List.of();
                                                        }))
                        .toList();
        List<ProviderQuote> merged = futures.stream().flatMap(f -> f.join().stream()).toList();
        cache.put(key, merged);
        return merged;
    }

    /**
     * Fanout, then sort by the caller-chosen policy. Returns a fresh mutable list — callers
     * sometimes pipe through further filtering.
     */
