 ...
 *
 * <p>Current strategy (MVP): - FedEx: Real quotes from FedEx Developer API - UPS, DHL, Private:
 * Mock quotes (for parity with legacy behavior)
 *
 * <p>Future: Add real UPS, DHL providers.
 */
@Service
public class QuoteService {
    ...
    /**
     * Re-generate quotes for an existing shipment request (by ID). Used by the Python AI service to
     * fetch services for a previously persisted request. Returns null if the shipment request is
     * not found.
     */
    public QuoteResponse regenerateQuotes(UUID shipmentRequestId) {
        ...
        ShipmentForQuote shipment =
                new ShipmentForQuote(
                        sr.getOrigin(),
                        sr.getDestination(),
    ...
    /**
     * Generate quotes for a shipment request. Persists the shipment request and returns a mix of
     * real (FedEx) and mock quotes.
     */
    public QuoteResponse generateQuotes(QuoteRequest request, String userId) {
        ...
        double totalWeight =
                request.packages().stream()
                        ...
                        .sum();
        int totalItems = request.packages().stream().mapToInt(p -> parseInt(p.qty())).sum();
    ...
    private void persistShipmentRequest(
            QuoteRequest request, String userId, double totalWeight, int totalItems) {
        try {
            ...
            log.debug(
                    "Persisted shipment request for {} -> {}",
                    request.origin(),
                    request.destination());
        } catch (Exception e) {
    ...
    private QuoteResponse buildQuotesWithRealProviders(
            ShipmentForQuote shipment, double totalWeight) {
        ...
        List<ShippingServiceDto> privateTop =
                List.of(luglessStandard(pm, baseDate), luggageToShipStandard(pm, baseDate));
        List<ShippingServiceDto> privateMore = List.of(luggageToShipEconomy(pm, baseDate));
    ...
    private ShippingServiceDto luggageToShipStandard(double pm, LocalDate baseDate) {
        return new ShippingServiceDto(
                "lts-std",
                "LuggageToShip",
                "LuggageToShip Standard",
                "STANDARD",
                round(54.0 * pm),
                null,
                5,
                fmt(baseDate.plusDays(5)),
                null,
                false,
                null,
    ...
    private ShippingServiceDto luggageToShipEconomy(double pm, LocalDate baseDate) {
        return new ShippingServiceDto(
                "lts-econ",
                "LuggageToShip",
                "LuggageToShip Economy",
                "ECONOMY",
                round(39.0 * pm),
                null,
                8,
                fmt(baseDate.plusDays(8)),
                null,
                false,
                null,
                "Most affordable.",
