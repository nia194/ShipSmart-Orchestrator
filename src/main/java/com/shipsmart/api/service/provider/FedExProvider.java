...
/**
 * FedEx shipping provider. Integrates with FedEx Developer API (Rate API v1). Handles OAuth2 token
 * management and quote fetching.
 *
 * <p>Requires environment variables: - FEDEX_CLIENT_ID, FEDEX_CLIENT_SECRET, FEDEX_ACCOUNT_NUMBER -
...
public class FedExProvider implements ShippingProvider {
    ...
    private static final Map<String, FedExService> FEDEX_SERVICES =
            Map.ofEntries(
                    ...
                    Map.entry(
                            "STANDARD_OVERNIGHT", new FedExService("FedEx Standard Overnight", 1)),
                    Map.entry(
                            "PRIORITY_OVERNIGHT", new FedExService("FedEx Priority Overnight", 1)),
    ...
    public List<ShippingServiceDto> getQuotes(ShipmentForQuote shipment) {
        try {
            ...
            double billableWeight =
                    shipment.totalWeight() > dimWeight ? shipment.totalWeight() : dimWeight;
    ...
    private Map<String, Object> buildRateRequest(ShipmentForQuote shipment, double billableWeight) {
        ...
        requestedShipment.put(
                "shipper",
                Map.of(
                        "address",
                        Map.of("postalCode", extractZip(shipment.origin()), "countryCode", "US")));
        ...
        requestedShipment.put(
                ...
                Map.of(
                        ...
                        Map.of(
                                "postalCode",
                                extractZip(shipment.destination()),
                                "countryCode",
                                "US")));
        ...
        packageItem.put("weight", Map.of("units", "LB", "value", round(billableWeight, 1)));
        packageItem.put(
                "dimensions",
                Map.of(
    ...
    private List<ShippingServiceDto> parseRateResponse(
            Map<String, Object> response, String dropOffDate) {
        ...
        try {
            ...
            List<Map<String, Object>> rateReplyDetails =
                    (List<Map<String, Object>>) output.get("rateReplyDetails");
            ...
            for (var rateDetail : rateReplyDetails) {
                ...
                FedExService fedexService =
                        FEDEX_SERVICES.getOrDefault(
                                serviceType, new FedExService("FedEx " + serviceType, 5));
                ...
                if (commit != null) {
                    Map<String, Object> transitDaysObj =
                            (Map<String, Object>) commit.get("transitDays");
    ...
    private synchronized void ensureToken() throws Exception {
        ...
        try {
            ...
            Map<String, Object> response =
                    restTemplate.postForObject(
                            tokenUrl, buildFormUrlEncodedRequest(body), Map.class);
