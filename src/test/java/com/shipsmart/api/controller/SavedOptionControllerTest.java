...
class SavedOptionControllerTest {
    ...
    @MockitoBean private SavedOptionService savedOptionService;
    ...
    void getSavedOptions_unauthenticated_returns401() throws Exception {
        mockMvc.perform(get("/api/v1/saved-options")).andExpect(status().isUnauthorized());
    }
    ...
    void getSavedOptions_authenticated_returns200() throws Exception {
        ShippingServiceDto svc =
                new ShippingServiceDto(
                        ...
                        Map.of(),
                        List.of());
        SavedOptionResponse resp =
                new SavedOptionResponse(
                        "id-1",
                        "ups-ground",
                        svc,
                        "NY",
        ...
        mockMvc.perform(get("/api/v1/saved-options").with(authentication(userAuth())))
    ...
    void removeSavedOption_authenticated_returns200() throws Exception {
        ...
        mockMvc.perform(delete("/api/v1/saved-options/opt-1").with(authentication(userAuth())))
                .andExpect(status().isOk())
    ...
    void removeSavedOption_unauthenticated_returns401() throws Exception {
        mockMvc.perform(delete("/api/v1/saved-options/opt-1")).andExpect(status().isUnauthorized());
    }
}
